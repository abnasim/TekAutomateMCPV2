...
export function getToolDefinitions() {
  return [
    ...
    },
    {
      ...
      description: 'Capture a screenshot from the live instrument. USE THIS TOOL PROACTIVELY — do not wait for the user to ask. Call it: at the start of a task to see current display state; after any change that affects what is visible on screen (channel enable/disable, scale, decode setup, trigger config); whenever a measurement returns an unexpected value or 9.9E37; and as the final verification step of every task. IMPORTANT: pass analyze:true to receive the scope image in this tool response — without it, the screenshot only refreshes the TekAutomate UI display and NO image data is returned to you. Always use analyze:true when you need to see, analyze, or verify the scope screen. ANALYSIS RULE: treat screenshots as authoritative for visible instrument state. If a screenshot shows clipping, off-screen traces, disabled channels, or missing decode/table visibility, fix that first — do not trust or report measurements from a visibly clipping channel. After enabling a channel, verify it is visible, framed, and not clipping before analyzing it.',
      parameters: {
        type: 'object',
        properties: {
          executorUrl: { type: 'string' },
